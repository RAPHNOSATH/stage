    ...
    <div class="card" >
        ...
        @if(count($equipes) > 0)
            ...
            <div class="d-flex gap-2 w-100 justify-content-between mt-2">
                <h4 >Les équipes de recherche</h4>
            </div>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if($equipe->effectif_min > 0)
                            <td style="padding: 30px !important">{{$equipe->effectif_min}} <a class="btn btn-primary ml-2" href="{{url('/suivi/equipe/membre1/'.$equipe->id)}}">Consulter la liste</a></td>
                        @else
                            ...
                            <div class="d-flex gap-2 w-100 justify-content-end">
                                <div class="form-check form-switch" style="padding: 20px !important">
                                    @if ($equipe->statut_equipe == 'Inactif'|| $equipe->statut_equipe == 'En pause')
                                        <a class="form-check-input" href="{{url('/suivi/activation1/'.$equipe->id)}}"></a>
                                        <label class="form-check-label" for="flexSwitchCheckChecked">Activer</label>
                                    @elseif($equipe->statut_equipe == 'Actif')
                                        <a class="form-check-input checkeds" href="{{url('/suivi/desactivation1/'.$equipe->id)}}"></a>
                                        <label class="form-check-label" for="flexSwitchCheckChecked">Désactiver</label>
                                    @endif
                                </div>
                                @if($equipe->statut_equipe == 'Actif')
                                    <a data-bs-custom-class="tooltip" data-bs-placement="bottom" data-bs-title="Mettre en pause l'équipe" href="{{url('/suivi/enpause1/'.$equipe->id)}}"><span class="badge bg-info rounded-pill" style="margin-top:5px; padding: 20px !important; font-size:20px"><i class="bi bi-pause-circle"></i></span></a>
                                @endif
                                <button data-bs-custom-class="tooltip" data-bs-placement="bottom" data-bs-title="Supprimer l'équipe avec ses membres" class="modalbutton" type="button" data-bs-toggle="modal" data-bs-target="#delete{{ $equipe->id }}"><span class="badge bg-danger rounded-pill" style="padding: 20px !important; font-size:20px"><i class="bi bi-trash"></i></span></button>
                                            ...
                                            <div class="modal-body">
                                                <div class="container">
                                                    ...
                                                    <section class="container mt-5" >
                                                        <form method="POST" action="{{url('/suivi/equipe1/'.$equipe->id)}}" enctype="multipart/form-data" class="row g-3 " style="background: #3333; margin-left:40px ">
                                                            @csrf
                                                            @method('delete')
                                                            <div class="col-12">
                                                            ...
                                                            <div class="col-md-4 mt-4">
                                                                ...
                                                                <select id="projet" class="form-control" name="projet" required autofocus autocomplete="projet">
                                                                    @forEach($equipe->projets as $projet)
                                                                        <option value="{{$projet->id}}" selected>{{$projet->intitule_projet}}</option>
                                                                    @endforeach
                                                                    @forEach($projets as $projet)
                                                                        @if($projet->equipe_id == null)
        ...
        @else
            <div class="fs-4 text-center text-primary mt-5">Pas d'équipe de recherche!!</div>
        @endif
        @elseif (auth()->check())
        @if(count($equipes) > 0)
            <div class="d-flex gap-2 w-100 justify-content-between mt-2">
