                    ...
                    <div class="d-flex gap-2 w-100 justify-content-end">
                        @if (auth('membre')->check())
                            @if(auth()->guard('membre')->user()->is_superuser && auth()->guard('membre')->user()->is_expert)
                                <a style="margin-left: 2px" href="{{url('/gestion/updateProjet1/'.$projet->id)}}" class="btn btn-success">Valider</a>
                            @endif
                        @elseif (auth()->check())
                            @if(auth()->user()->is_superuser)
                                <a style="margin-left: 2px" href="{{url('/gestion/updateProjet/'.$projet->id)}}" class="btn btn-success">Valider</a>
                            @endif
                        @endif
                    </div>
                </td>
